...
<html lang="en">
    ...
    <body>
        <?php 
            ...
            if($_GET['id']){
                $id=$_GET['id'];
                $data=mysqli_query($connect,"select * from sanpham  where mathuonghieu='$id'");
        ...
        <section class="py-5">
            <?php 
                while($rowcat=mysqli_fetch_assoc($category)){
                    echo '<p style="font-size: 30px;font-weight:bold;margin-left:30px">Tất cả sản phẩm '. $rowcat['name'].'</p>';
                }
            ?>
