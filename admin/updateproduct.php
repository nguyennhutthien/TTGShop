...
<head>
  ...
  <?php 
    ...
    if(isset($_GET['id'])){
        $id=$_GET['id'];
        $dataupdate=mysqli_query($connect,"select * from sanpham where id='$id'");
    ...
    if(isset($_POST['update'])){
        ...
        if($name!="" && $file_name!="" && $price!="" && $description!=""){
          $data=mysqli_query($connect,"UPDATE `sanpham` SET name='$name',image='$file_name',price='$price',description='$description',thongso='$thongso',mathuonghieu='$brand',madanhmuc='$category' WHERE id ='$idUpdate'");
          if($data){
  ...
  <div class="content-wrapper">
    ...
    <form action="updateproduct.php" method="post" enctype="multipart/form-data">
      <div class="card-body">
        ...
        <label for="">ID</label>
        <input type="text" class="form-control" placeholder="ID" name="masp" value="<?php echo $rowupdate['id'] ?>"><br>
        <label for="">Tên sản phẩm</label>
        <input type="text" class="form-control" placeholder="Tên sản phẩm" name="name" value="<?php echo $rowupdate['name'] ?>"><br>
        <label for="">Hình sản phẩm</label>
        <input type="file" class="form-control" name="image" accept=".png, .jpg, .ico"><br>
        <label for="">Giá</label>
        <input type="number" class="form-control" name="price" value="<?php echo $rowupdate['price'] ?>"><br>
        <label for="">Danh mục:</label>
        <select name="category" id="category" class="form-control">
            <?php 
                $category=mysqli_query($connect,"select * from danhmuc");
                while($rowcategory=mysqli_fetch_assoc($category)){
                    if($rowcategory['id']==$rowupdate['madanhmuc']){
        ...
        <select name="brand" id="brand" class="form-control">
            <?php 
                ...
                while($rowbrand=mysqli_fetch_assoc($brand)){
                    if($rowbrand['id']==$rowupdate['mathuonghieu']){
        ...
        <textarea class="form-control" name="description" id="description" value="<?php echo $rowupdate['description'] ?>"></textarea><br>
        <label for="">Thông số kỹ thuật:</label>
        <textarea class="form-control" name="thongso" id="thongso" value="<?php echo $rowupdate['thongso'] ?>"></textarea><br>
        </div>
